...
func (b *llbBridge) loadResult(ctx context.Context, def *pb.Definition, cacheImports []gw.CacheOptionsEntry, pol []*spb.Policy) (solver.CachedResultWithProvenance, error) {
	...
	if err != nil {
		fmt.Printf("got an error when calling build on def=%p digest=%s; gonna stall here", def, edge.Vertex.Digest())
...
func graphToString(prefix string, edge solver.Edge) string {
	s := fmt.Sprintf("%sdgst=%s %s\n", prefix, edge.Vertex.Digest(), edge.Vertex.Name())
	for _, e := range edge.Vertex.Inputs() {
		s += graphToString(prefix+"  ", e)
	}
	return s
}
